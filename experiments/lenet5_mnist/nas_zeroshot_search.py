#!/usr/bin/env python3
"""
NAS board search (training-free ranking)  —  LeNet-5 / MNIST
============================================================

The idea (the "bet"): we never train during the search.  Instead we

  1. SAMPLE board-fitting configs with the library's own NAS generator
     (`get_nas_compression_data`, train=False) — each config is compressed,
     calibrated and evaluated ZERO-SHOT (no fine-tuning), and a geometry filter
     keeps only those that fit the board's SRAM + Flash budget.
  2. RANK them by that zero-shot accuracy.
  3. TRAIN only the top-k fully (2-stage: prune -> QAT) and keep the best.

If a better zero-shot config really is a better trained config, the top-k will
contain the winner — so we get a deployable model for the price of k trainings
instead of training the whole search space.

Run:
    python experiments/lenet5_mnist/nas_zeroshot_search.py \
        --n_sample 250 --n_train 8 --prune_epochs 15 --qat_epochs 12
"""
...
from development.experiments.mnist  import get_metric
...
# Same baseline + data pipeline as the verified W4A4 reproduction
# (reproduce_table1) so we climb toward the same measured ~96% Uno ceiling.
BASELINE_CKPT = os.path.join(
    PROJECT_ROOT, "experiments/reproduce_table1/lenet5_state_dict.pth")
...
# ─────────────────────────────────────────────────────────────────────────────
# Data  (light aug + batch 32 — the recipe the proven LeNet result was tuned on)
# ─────────────────────────────────────────────────────────────────────────────
def get_loaders(batch_size: int = 32, eval_batch_size: int = 256):
    ...
    train_ds = datasets.MNIST(DATASET_DIR, train=True,  download=True, transform=train_tf)
    test_ds  = datasets.MNIST(DATASET_DIR, train=False, download=True, transform=test_tf)
    return (
        torch_data.DataLoader(train_ds, batch_size=batch_size,      shuffle=True,  **kw),
        torch_data.DataLoader(test_ds,  batch_size=eval_batch_size, shuffle=False, **kw),
    )
...
def config_at(data: dict, index: int) -> dict:
    """Pull one flat compression config (drop the 'metric' column) out of the
    parallel-list dict that get_nas_compression_data returns."""
...
def finetune(model, train_loader, test_loader, epochs: int, lr: float):
    """Plain CE + SGD + ReduceLROnPlateau (fit() passes the val loss to step)."""
    crit = nn.CrossEntropyLoss()
    opt  = optim.SGD(model.parameters(), lr=lr, weight_decay=5e-4, momentum=0.9)
    sch  = optim.lr_scheduler.ReduceLROnPlateau(opt, mode="min", patience=1)
...
def train_config(decoded_cfg: dict, train_loader, test_loader,
    ...
    """Train one decoded compression config end-to-end and return its metrics.

    Stage A — prune the baseline and fine-tune in FP32 (channels recover here).
    Stage B — quantize the pruned model and run QAT.
    Passing the full (prune+quant) config to the already-pruned model is exactly
    what the verified reproduction does — pruning is re-applied as the same mask,
    so it is idempotent."""
    base = load_baseline()

    # Stage A — structured pruning + FP32 fine-tune
    prune_cfg = {"prune_channel": decoded_cfg["prune_channel"]}
    pruned = base.init_compress(prune_cfg, INPUT_SHAPE).to(DEVICE)
    finetune(pruned, train_loader, test_loader, prune_epochs, lr=1e-3)
    pruned_acc = pruned.eval().evaluate(test_loader, METRICS, device=DEVICE)["top1acc"]

    # Stage B — quantize the pruned model + QAT
    calib = next(iter(test_loader))[0].to(DEVICE)
    q = pruned.init_compress(decoded_cfg, INPUT_SHAPE, calib).to(DEVICE)
    finetune(q, train_loader, test_loader, qat_epochs, lr=1e-4)
...
def main():
    ...
    ap.add_argument("--sram",  type=int, default=2048)
    ap.add_argument("--flash", type=int, default=32768)
    ap.add_argument("--n_sample", type=int, default=250, help="board-fitting configs to collect")
    ap.add_argument("--n_train",  type=int, default=8,   help="top configs to train")
    ap.add_argument("--prune_epochs", type=int, default=15)
    ap.add_argument("--qat_epochs",   type=int, default=12)
    ...
    if tr:
        ...
        print(f"\n{'='*64}\n  BEST DEPLOYABLE LeNet-5 (zero-shot search)\n{'='*64}")
