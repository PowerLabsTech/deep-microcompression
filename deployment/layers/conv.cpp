...
uint32_t Conv2d::get_output_size(void) {
    ...
    read_conv_header(&p,
        ...
        &kernel_row_size, &kernel_col_size, &padding_row, &padding_col,
        &stride_row, &stride_col, &groups);
    uint16_t output_row_size = (input_row_size + 2 * padding_row - kernel_row_size) / stride_row + 1;
    uint16_t output_col_size = (input_col_size + 2 * padding_col - kernel_col_size) / stride_col + 1;
    return (uint32_t)output_channel_size * output_row_size * output_col_size;
}
            ...
            for (uint16_t l = 0; l < output_col_size; l++) {
                ...
                for (uint16_t c_out = 0; c_out < output_channel_per_group; c_out++) {
                    ...
                    activation_write_float(workspace_start,
                        (m * output_col_size * output_channel_size) + (l * output_channel_size) + n,
                        bias ?
                        (pixel[c_out] * parameter_read_float(weight_scale, scale_index) + parameter_read_float(bias, n)) :
                        (pixel[c_out] * parameter_read_float(weight_scale, scale_index)));
                }
            }
        }
    }
